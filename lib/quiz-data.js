...
export function getImagemPath(numeroQuestao) {
  ...
  // Calcular o número da questão dentro da série (1-12)
  if (numeroQuestao >= 1 && numeroQuestao <= 12) {
    numeroNaSerie = numeroQuestao; // Série A: 1-12
  } else if (numeroQuestao >= 13 && numeroQuestao <= 24) {
    numeroNaSerie = numeroQuestao - 12; // Série B: 1-12
  } else if (numeroQuestao >= 25 && numeroQuestao <= 36) {
    numeroNaSerie = numeroQuestao - 24; // Série C: 1-12
  } else if (numeroQuestao >= 37 && numeroQuestao <= 48) {
    numeroNaSerie = numeroQuestao - 36; // Série D: 1-12
  } else if (numeroQuestao >= 49 && numeroQuestao <= 60) {
    numeroNaSerie = numeroQuestao - 48; // Série E: 1-12
  } else {
    numeroNaSerie = 1; // fallback
  }

  return `/images/${serie}${numeroNaSerie}.webp`;
}
